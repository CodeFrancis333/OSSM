...
export default function SectionEditor({ selectedDia: propDia, setSelectedDia: propSetDia, length: propLength, setLength: propSetLength, count: propCount, setCount: propSetCount, addBomLine }){
  ...
  useEffect(()=>{
    ...
    function parseDia(mmKey){
      const n = Number(String(mmKey).replace('mm',''))
      return isNaN(n) ? 0 : n
    }
  
    function validateSpacing(){
      const dia = parseDia(propDia ?? localDia)
      const sMin = detailing.S_min_mm || 25
      const errors = []
      if (spacing < sMin) errors.push(`Spacing ${spacing}mm < S_min ${sMin}mm`)
      if (spacing < dia) errors.push(`Spacing ${spacing}mm < bar dia ${dia}mm`)
      return errors
    }
    const errors = validateSpacing()
